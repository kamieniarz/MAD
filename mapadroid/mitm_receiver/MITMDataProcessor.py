    ...
    def process_data(self, received_timestamp, data, origin):
        ...
        if data_type and not raw:
            ...
            if data_type == 106:
                ...
                logger.debug("Processing GMO received from {}. Received at {}", str(
                ...
                logger.success("GMO przetworzone - {}".format(origin))
            if data_type == 101:
                logger.debug2("Processing proto 101 of {}".format(origin))
                self.__db_submit.quest(origin, data["payload"], self.__mitm_mapper)
                logger.success("Quest zeskanowany - {}".format(origin))
            logger.debug("Ignoruje pozostale dane - {}", origin)
            return
